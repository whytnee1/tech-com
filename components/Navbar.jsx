  ...
  return (
    <main className="flex items-center justify-between px-6 py-0 shadow-md sticky top-0 w-full bg-white z-50">
      ...
      <div className={`md:hidden bg-white h-dvh w-full absolute top-0 left-0 ${navOpen ? "flex" : "hidden"} flex-col items-center gap-10 pt-20`}>
        ...
        {/* Profile Element inside Mobile View under Contact Us */}
        {session ? (
          <div>
            <button
          ...
          <Link href={"/signin"} onClick={() => setNavOpen(false)}>
      ...
      {/* Desktop and Tablet Profile Section */}
      {session ? (
        <div className="max-md:hidden">
          <button
            id="desktop-basic-button"
            aria-controls={open ? 'basic-menu' : undefined}
            aria-haspopup="true"
            aria-expanded={open ? 'true' : undefined}
            onClick={handleClick}
      ...
      {/* Shared MUI Menu Controller (Both viewpoints connect here dynamically) */}
      <Menu
        id="basic-menu"
        anchorEl={anchorEl}
        open={open}
        ...
        slotProps={{
          list: {
            'aria-labelledby': anchorEl?.id || 'desktop-basic-button',
          },
        }}
      >
